(function () {
    ...
    function colloquiumSessionDetailCtrl($q,$scope, $stateParams, $window, common, config, datacontext) {
        ...
        vm.colloquiumAutocompleteOptions = ({
            minLength: 1,
            dataSource: new kendo.data.DataSource({
                transport: {
                    read: function (options) {
                        return vm.getColloquiums(options, vm.selectedColloquium);
                    }
                }
            })
        });
        ...
        function getColloquiums(options,value) {
            return datacontext.colloquium.getTypeaheadData(value).then(function(colloquiums) {
                var colloquiumsRetrieved=colloquiums.map(function(colloquium) {
                    return colloquium.period;
                });
                options.success(colloquiumsRetrieved);
            });
        }
